...
class ExportHandling:
    ...
    def load_video_to_numpy(self, video_path="/Users/fryderykkogl/Documents/university/master/thesis/data.nosync/nick/test/IM00001.mov"):
        """
    ...
    def crop_array(self, np_array, left=0.0, right=0.0, top=0.0, bottom=0.0):
        """
    ...
    def export_array_to_video(self, np_array, save_path="/Users/fryderykkogl/Documents/university/master/thesis/data.nosync/nick/test/IM00001_crop.mov", codec='H264', fps=30):
        """
    ...
    def one_video(self, video_path, save_path):
        """
        ...
        us_numpy = self.load_video_to_numpy(video_path)

        # deidentify by cropping
        us_numpy_cropped = self.crop_array(us_numpy.copy(), top=0.09166)

        # create video
        self.export_array_to_video(us_numpy_cropped, save_path=save_path)

    def run_export_main(self, load_paths):

        # set up logging
        if load_paths[0].lower().endswith(".mov"):
            logging.basicConfig(filename=load_paths[0].lower().replace(".mov", ".log"))
        else:
            logging.basicConfig(filename=load_paths[0].lower().replace(".mp4", ".log"))
        logger = logging.getLogger()
        logger.setLevel(logging.DEBUG)

        for load_path in load_paths:
            # get paths
            assert load_path.lower().endswith(".mov") or load_path.lower().endswith(".mp4"), "File has to be .mov or .mp4"
            load_path = load_path
            ...
            try:
                self.one_video(load_path, save_path)
